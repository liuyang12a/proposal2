...
围绕交通流预测问题，出现了大量的研究工作，主要是通过训练时空图神经网络模型的方法来提取序列的时空特征。
...
\begin{longtable}{|m{0.17\textwidth}|m{0.18\textwidth}|m{0.15\textwidth}|m{0.20\textwidth}|m{0.15\textwidth}|}
    ...
    \hline
    \fangsong GeoMAN\upcite{liang2018geoman} & \fangsong None & \fangsong Attn & \fangsong RNN+Attn & \fangsong Weather+POI
\end{longtable}
\normalsize

交通流预测的关键挑战在于空间拓扑构造、空域特征提取、时域特征提取以及场景额外特征使用四个方面，因此当前主流的方法大都着眼于寻求好的信息融合方法来提高预测的准确率。

在空间拓扑构造上，传统方法时间序列预测方法（如HA、SVR、ARIMA等）独立的处理不同节点处的时序变化，因此不需要预先构造网络拓扑。
当使用卷积神经网络（CNN）进行特征提取时，通常以网格化的方式来构造空间拓扑，即按区域将地理位置划分为规则的网格。
此外，图神经网络打破了限制，可以用于非欧几里德数据，更通用和灵活地描述现实世界的道路网络。
原始的图构造方法是通过带阈值高斯核函数的距离度量来计算节点对之间的相似度，得到加权邻接矩阵，或者只是使用连通性来推导一个简单的二值矩阵。
一些其他的工作还用从多个不同视角来构造空间拓扑，如兴趣点（POI）相似度、DTW 相似度、自由流可达相似度、带宽图等等。
然而，预定义的邻接矩阵是静态的，且构造方法有限，这是描述复杂路网的天然缺陷。
为了弥补“丢失的视图”，采用自适应邻接矩阵的方法被提出，并实现了邻接矩阵参数随训练过程可学习的性能改进。
然而，静态自适应邻接矩阵仍然难以模拟道路网络的动态特性。
在相邻的时间步长将所有节点与自己连接起来，得到一个局部的时空图，从中可以直接捕获每个节点与其时空邻居之间的相关性。但是，该方法只考虑最接近的时间步长，表示能力有限。

在空域特征提取上，传统的方法只关注单个变量的时间序列，未能捕捉变量之间的相关性，这也严重限制了面对多变量时间序列时的效率。
此外，传统的统计方法大多需要很强的与平稳相关的假设，这导致对波动大、缺失值多的时间序列的预测极端失败。近年来，随着深度学习的发展，卷积神经网络被广泛应用于从欧几里得数据中获取空间相关性。
图嵌入可以捕获空间依赖关系，生成编码的嵌入向量供后续处理。
图神经网络在捕获非欧数据中的空间相关性方面具有强大的灵活性。

在时域特征提取上，循环神经网络（RNN）及其变体是针对顺序数据提出的，并且在捕获时间依赖性方面具有优势。
循环神经网络的循环运算以消耗时间和内存为代价，提高了模型结构的灵活性。
LSTM\upcite{hochreiter1997long} 和 GRU\upcite{chung2014empirical} 的出现进一步增强了对较长序列依赖\upcite{bengio1994learning}的建模能力和避免梯度消失的能力。
卷积神经网络已被证明在时序数据中捕获时间相关性是有效和高效的，但其架构相对比较稳固，适应不同情况的调整空间很小。
注意力机制（Attn）在顺序依赖建模中也很强大和有用，包括自注意力和其他变体。
以Transformer\upcite{vaswani2017attention}为代表，基于自注意力机制的模型已经统治了自然语言处理领域。
基于自注意力的模型虽然具有全局顺序依赖和并行化的能力，但仍然存在内存消耗大、对计算源要求高的问题。
此外，过于简单的位置编码不足以表示复杂的序列信息，而自注意力只能计算出重要性权重，因此基于自注意力的模型比循环神经网络更难捕获局部（短期）序列依赖。

在额外特征上，主要基于场景特有的先验知识来辅助预测。例如在交通运输场景下，季节特征、兴趣点特征、道路网特征以及工作日节假日特征等，作为先验知识能够为预测提供额外的信息。

除了对交通流的预测外，还有一些对于系统状态的预测工作。例如，Liu等人\upcite{liu2024deep}研究复杂系统的韧性时，以系统的网络拓扑以及演化的时空序列为输入，利用深度学习的方法来推断系统是否达到稳态、以及是否会偏离稳态。
