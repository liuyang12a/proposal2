...
Sperduti等人\upcite{sperduti1997supervised}最开始使用有监督学习的神经网路来解决网络结构的分类问题，使用的就是循环迭代的思路，但受限于当时计算机的性能，能够处理的网络结构被限定为有向无环图，而且该项工作也普遍被认为是最早的图神经网络的研究之一\upcite{velickovic2017graph,wu2020comprehensive}。
...
\paragraph{自编码图神经网络}
Aditya等人\upcite{grover2016node2vec}提出了一种名为Node2Vec的算法框架，借鉴了自然语言处理中Word2Vec的表示学习思路，以一种灵活的节点网络邻域定义和有偏随机游走策略，能够探索多样化的邻域结构。
当邻域结构确定后，对网络结构的表示学习过程即对应于优化以下目标：
\begin{equation}
    \max_{f} \quad \sum_{u\in V} \log \textit{Pr}(\textit{N}_{\textit{S}}(u)|f(u))
\end{equation}
其中，$\textit{N}_{\textit{S}}(u)$表示节点$u$的邻域，基于邻域的条件独立性和对称性假设有：
\begin{equation}
    \begin{aligned}
        \textit{Pr}(\textit{N}_{\textit{S}}(u)|f(u))= \prod_{n_i \in \textit{N}_{\textit{S}}(u)} \textit{Pr}(n_i|f(u)) \\
        \textit{Pr}(n_i|f(u))=\frac{\exp(f(n_i)\cdot f(u))}{\sum_{v\in V}\exp(f(v)\cdot f(u))}
    \end{aligned}
\end{equation}
此时，目标函数可以转化为：
\begin{equation}
    \begin{aligned}
            &\max_{f} \quad \sum_{u\in V} \Big[ \sum_{n_i \in \textit{N}_{\textit{S}}(u)} \log \textit{Pr}(n_i|f(u)) \Big] \\
            \Rightarrow &\max_{f} \quad \sum_{u\in V} \Big[ \sum_{n_i \in \textit{N}_{\textit{S}}(u)} \log \frac{\exp(f(n_i)\cdot f(u))}{\sum_{v\in V}\exp(f(v)\cdot f(u))} \Big] \\
            \Rightarrow &\max_{f} \quad \sum_{u\in V} \Big[ -\log Z_u + \sum_{n_i \in \textit{N}_{\textit{S}}(u)} f(n_i)\cdot f(u)) \Big]
    \end{aligned}
\end{equation}
...
交通流量的预测，可以被抽象定义为时空序列预测问题，其输入包括：交通网络$G(V,E)$，以及历史交通信号序列$（X_1, X_2,...,X_T）\in \mathbb{T\times |V|\times C}$，其中$T$表示在不同时刻的采样点的数量，$C$表示采样特征的通道数量。输出为预测的未来$t$个时刻的交通信号：$(X_{T+1}, X_{T+2},...X_{T+t})\in \mathbb{t\times |V|\times C}$
...
随着以Transformer结构\upcite{vaswani2017attention}中的多头注意力机制在序列表示学习任务中展现出的优越性能，基于注意力机制的时序特征表示也被逐渐用于时空序列预测的模型管道当中\upcite{aguo2019attention,guo2021learning}。
...
在大数据时代，研究者往往能获取节点状态的时间序列数据，却难以直接观测节点间的连接结构与动力学规则，这一困境催生了复杂网络重构（Reconstruction methods for networks）这一前沿交叉方向。其核心目标是通过数据驱动的算法，从观测数据中逆向推断网络拓扑结构、动力学演化规律及潜在交互机制，为复杂系统的理解、预测与控制提供基础。
网络重构问题可细分为三类：仅推断动力学方程的动力学重构、仅恢复吸引子的相空间重构，以及同时推断结构与动力学的完整重构，其中后者是当前研究的核心挑战。

动态网络重构可以归结为一类特殊的网络重构问题，其可观测的信息主要是各节点的动态演化过程，从而以此来推断这些节点所组成的交互网络。
因此，动态网络重构问题可视为时空序列分析问题的逆向过程。
